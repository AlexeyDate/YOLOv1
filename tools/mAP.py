        ...
        for i in range(len(pred_boxes)):
            ...
            for k in range(len(pred_boxes_class)):
                ...
                for j in range(len(true_boxes_class)):
                    if intersection_over_union(pred_boxes_class[k][1:5], true_boxes_class[j][1:5]) > max_iou:
                        max_iou = intersection_over_union(pred_boxes_class[k][1:5], true_boxes_class[j][1:5])
                        max_index = j
